...
@section('tan-main')
    <section class="scrollable wrapper">
        <section class="panel panel-default">
                ...
                <table class="table table-striped b-t b-light">
                    ...
                    @forelse($tags as $tag)
                        <tr>
                            ...
                            <td>
                                <a href="/ad/show/role/{{base64_encode($tag->id)}}">{{$tag->display_name}}</a>
                            </td>
                            <td>{{$tag->name}}</td>
                            <td>
                                <div class="doc-buttons">
                                    <a href="/ad/show/role/{{base64_encode($tag->id)}}">
                                        <i class="fa fa-edit text-primary-lter"> 编辑</i>
                                    </a>
                                    <a href="#" class="oper-del" data-id="{{base64_encode($tag->id)}}"
                                       data-name="{{$tag->display_name}}" data-url="/ad/destroy/role"><i
                                                class="fa fa-trash-o text-primary-lter"> 删除</i></a>
                                    <a href="/ad/dispatch/auth/{{base64_encode($tag->id)}}">
            ...
            @include('tan-admin.partials.pagination' , ['paginator' => $tags])
        </section>
    </section>
@endsection
